...
def run_grid_search_alpha_lambda(X, y, alpha_grid, lambda_grid, best_gamma, k, penalty, labeled_ratio, unlabeled_ratio,   n_neighbors, m_ratio, random_state=None):    
    OA_matrix = np.zeros((len(lambda_grid), len(alpha_grid)))
    best_OA = -np.inf
    best_alpha = None
    best_lambda = None
    ...
    for i, lam in enumerate(lambda_grid):
        for j, al in enumerate(alpha_grid):
            ...
            Beta, convergence = solve_beta(
                ...
                alpha=al, lambda_reg=lam, gamma=best_gamma,
            ...
            OA_matrix[i, j] = acc
            print(f"lambda={lam:.2f}, alpha={al:.2f} => OA={acc:.4f}")

            if acc > best_OA:
                best_OA = acc
                best_alpha = al
                best_lambda = lam
    return OA_matrix, best_alpha, best_lambda, best_OA